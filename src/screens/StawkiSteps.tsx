...
// 13.09.2026: экран переведён со StawkiBet на 1xBet. Палитра взята с
// официальной иконки конторы (#276BA6). Прежняя золотая версия целиком
// сохранена рядом в StawkiSteps.stawkibet.tsx.bak — структура та же,
// если понадобится вернуться, достаточно переименовать файл обратно.
const BLUE      = '#4C9FE8'                 // акцент на тёмном фоне
const BLUE_DEEP = '#276BA6'                 // фирменный синий с логотипа
const GOLD      = BLUE                      // имя оставлено, чтобы не трогать разметку
const GOLD_DIM  = 'rgba(76,159,232,.13)'
const GOLD_LINE = 'rgba(76,159,232,.30)'
  ...
  return (
    <M.button whileTap={{ scale: .97 }} onClick={() => { haptic('light'); copy() }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5,
        ...
        background: copied ? GOLD : 'rgba(76,159,232,.16)',
        transition: 'background .2s' }}>
        {copied
          ? <svg width="11" height="8" viewBox="0 0 11 8" fill="none">
              <path d="M1 4L4 7L10 1" stroke="#FFFFFF" strokeWidth="1.8"
        ...
        <span style={{ fontFamily: mono, fontSize: 8, fontWeight: 700,
          color: copied ? '#0B2135' : GOLD, letterSpacing: '.06em' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: 9, flexShrink: 0,
              background: 'rgba(76,159,232,.12)', border: `1px solid rgba(76,159,232,.26)`,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: 9, flexShrink: 0,
              ...
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
                ...
                <path d="M7.6 8.3h8.8M7.6 11h8.8M12 11v5.6"
                  stroke="#34D399" strokeWidth="1.6" strokeLinecap="round"/>
              </svg>
            </div>
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,.55)',
              lineHeight: 1.3, fontFamily: 'sans-serif' }}>
              Первый депозит от 5000 тнг ≈ $11 (выведешь когда захочешь)
            </span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: 9, flexShrink: 0,
              background: 'rgba(76,159,232,.12)', border: `1px solid rgba(76,159,232,.30)`,
              display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <M.a href={STAWKI_URL} target="_blank" rel="noreferrer" whileTap={{ scale: .97 }}
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
            ...
            background: `linear-gradient(135deg,${BLUE_DEEP} 0%,#5FB0F2 52%,${BLUE_DEEP} 100%)`,
            boxShadow: `0 0 32px ${GOLD}55, 0 4px 18px rgba(0,0,0,.5)`,
            textDecoration: 'none', cursor: 'pointer',
            fontFamily: f, fontWeight: 900, fontSize: 15, color: '#FFFFFF',
          ...
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <rect x="3" y="6" width="18" height="13" rx="2.5" stroke="#FFFFFF" strokeWidth="1.8"/>
            <path d="M3 10h18" stroke="#FFFFFF" strokeWidth="1.8"/>
            <circle cx="7.5" cy="14" r="1.5" fill="#FFFFFF"/>
          </svg>
          Открыть счёт на 1xBet
        </M.a>
          ...
          <div style={{ width: 26, height: 26, borderRadius: 9, flexShrink: 0,
            ...
            {depositDone && (
              <svg width="13" height="10" viewBox="0 0 13 10" fill="none">
                <path d="M1 5L5 9L12 1" stroke="#FFFFFF" strokeWidth="2.2"
                  strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            )}
          </div>
      ...
      <M.button whileTap={canSubmit ? { scale: .97 } : {}} onClick={canSubmit ? () => { haptic('medium'); onSubmit() } : undefined}
        style={{ width: '100%', padding: '16px', borderRadius: 16, border: 'none',
          ...
          background: canSubmit
            ? `linear-gradient(135deg,${BLUE_DEEP} 0%,#5FB0F2 52%,${BLUE_DEEP} 100%)`
            : 'rgba(255,255,255,.05)',
          boxShadow: canSubmit ? `0 0 28px ${GOLD}55` : 'none',
          fontFamily: f, fontWeight: 900, fontSize: 15,
          color: canSubmit ? '#FFFFFF' : 'rgba(255,255,255,.12)',
  ...
  return (
    <M.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      style={{ height: '100%', display: 'flex', flexDirection: 'column',
        background: '#061524', overflow: 'hidden', position: 'relative' }}>

      {/* Ambient */}
      <div style={{ position: 'absolute', top: '10%', left: '25%', transform: 'translateX(-50%)',
        width: 300, height: 260, pointerEvents: 'none',
        background: 'radial-gradient(ellipse,rgba(76,159,232,.16) 0%,transparent 65%)',
        filter: 'blur(14px)' }} />
      <div style={{ position: 'absolute', top: '40%', right: '-5%',
        width: 240, height: 220, pointerEvents: 'none',
        background: 'radial-gradient(ellipse,rgba(39,107,166,.20) 0%,transparent 65%)',
        filter: 'blur(14px)' }} />
      ...
      <div style={{ flexShrink: 0, padding: 'var(--header-top) 22px 0',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <img src={bkLogo} width={30} height={30} alt="1xBet"
            style={{ borderRadius: 8, flexShrink: 0 }} />
          <div>
            <div style={{ fontFamily: f, fontWeight: 800, fontSize: 13, lineHeight: 1 }}>1xBet</div>
